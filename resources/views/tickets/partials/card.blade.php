<div class="ticket-card {{ $ticket->status }} ticket-list-item {{ $ticket->is_pinned ? 'is-pinned' : '' }} {{ $ticket->is_favorite ? 'is-favorite' : '' }}" data-ticket-id="{{ $ticket->id }}" style="position: relative; padding-top: 24px;">
    ...
    <div class="ticket-student-col" style="flex: 0 0 250px; min-width: 0; display: flex; align-items: center; justify-content: space-between; gap: 10px; width: 100%;">
        <div style="display: flex; align-items: center; gap: 10px; min-width: 0; flex-grow: 1;">
            ...
            <div style="display: flex; flex-direction: column; align-items: center; gap: 8px; flex-shrink: 0;">
                {{-- Pin/Unpin Button --}}
                <form method="POST" action="{{ route('tickets.pinned', $ticket) }}" style="display: inline; line-height: 1; margin: 0;">
                    @csrf
                    <button type="submit" style="background: none; border: none; cursor: pointer; color: {{ $ticket->is_pinned ? '#f59e0b' : '#d1d5db' }}; font-size: 1.1rem; padding: 2px; line-height: 1;" title="{{ $ticket->is_pinned ? 'Lepas Sematan' : 'Sematkan Tiket' }}">
                        <i class="fa-solid fa-thumbtack" style="{{ $ticket->is_pinned ? '' : 'transform: rotate(-45deg);' }}"></i>
                    </button>
                </form>
                {{-- Favorite Star Button --}}
                <form method="POST" action="{{ route('tickets.favorite', $ticket) }}" style="display: inline; line-height: 1; margin: 0;">
                    @csrf
                    <button type="submit" style="background: none; border: none; cursor: pointer; color: {{ $ticket->is_favorite ? '#f59e0b' : '#d1d5db' }}; font-size: 1.1rem; padding: 2px; line-height: 1;" title="{{ $ticket->is_favorite ? 'Batal Favorit' : 'Jadikan Favorit' }}">
                ...
                @if(auth()->user()->isSiswa() && $ticket->anonymous)
                    ...
                    <div style="min-width: 0; display: flex; flex-direction: column; gap: 2px;">
                        ...
                        <div class="desktop-status-badges" style="display: flex; align-items: center; gap: 6px; flex-wrap: wrap; margin-top: 2px;">
                            <span class="badge badge-{{ $ticket->status }} ticket-status-badge" style="margin: 0; width: fit-content; padding: 2px 8px; font-size: 10px; border-radius: 4px; line-height: 1.2; font-weight: 700;">{{ $ticket->status_label }}</span>
                            @if($ticket->anonymous)
        ...
        @if($ticket->student)
            <div class="mobile-status-badges" style="display: none; flex-direction: column; align-items: flex-end; gap: 4px; flex-shrink: 0;">
                <span class="badge badge-{{ $ticket->status }} ticket-status-badge" style="margin: 0; width: fit-content; padding: 3px 8px; font-size: 10px; border-radius: 4px; line-height: 1.2; font-weight: 700; white-space: nowrap;">{{ $ticket->status_label }}</span>
                @if($ticket->anonymous)
    ...
    <div style="flex: 1; min-width: 0; display: flex; flex-direction: column; gap: 6px;">
        ...
        <div style="display: flex; flex-direction: column; gap: 2px;">
            ...
            @if($ticket->status === 'dibatalkan' && $ticket->cancel_reason)
                <div style="margin-top: 6px; font-size: 12px; color: #ef4444; background: #fef2f2; border: 1px solid #fecaca; padding: 6px 10px; border-radius: 6px; text-align: left; width: fit-content; max-width: 100%; white-space: normal; word-break: break-word;">
    ...
    <div style="display: flex; gap: 8px; justify-content: flex-end; align-items: center; flex-shrink: 0; position: relative;">
        <button type="button" 
                class="btn btn-secondary btn-sm btn-detail-ticket" 
        ...
        @if($ticket->status !== 'selesai' && $ticket->status !== 'dibatalkan')
            <a href="{{ route('chat.show', $ticket) }}" class="btn btn-primary btn-sm btn-balas" data-ticket-id="{{ $ticket->id }}" style="padding: 6px 12px; font-size: 12px; margin: 0; display: inline-flex; align-items: center; gap: 4px; position: relative;">
                <i class="fas fa-comments"></i> Balas
                @if($ticket->unread_count > 0)
                    <span class="unread-badge" style="position: absolute; top: -5px; right: -5px; background: #ef4444; color: white; border-radius: 50%; width: 16px; height: 16px; font-size: 9px; font-weight: 700; display: flex; align-items: center; justify-content: center; border: 2px solid white; line-height: 1;">
        ...
        @if(auth()->user()->isGuru() && $ticket->status !== 'selesai' && $ticket->status !== 'dibatalkan')
            <div class="ticket-actions-dropdown" style="position: relative; display: inline-block;">
                <button type="button" class="btn btn-secondary btn-sm dropdown-trigger" style="padding: 6px 10px; font-size: 12px; margin: 0; display: inline-flex; align-items: center; justify-content: center; height: 28px; background: transparent; border: 1px solid var(--slate-light, #cbd5e1); color: var(--slate);" onclick="toggleActionsDropdown(event, '{{ $ticket->id }}')">
                    <i class="fas fa-ellipsis-v"></i>
                </button>
                <div class="dropdown-menu-content" id="actions-dropdown-{{ $ticket->id }}" style="display: none; position: absolute; right: 0; top: calc(100% + 5px); background: white; border: 1px solid #e2e8f0; border-radius: 8px; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05); z-index: 100; min-width: 170px; overflow: hidden; text-align: left;">
                    <a href="#" style="display: flex; align-items: center; gap: 8px; padding: 10px 14px; font-size: 13px; color: #d97706; text-decoration: none; transition: background 0.2s; white-space: nowrap;" onmouseover="this.style.background='#fffbeb'" onmouseout="this.style.background='transparent'" onclick="event.preventDefault(); closeAllDropdowns(); openSelesaiModal('{{ $ticket->id }}', '{{ addslashes($ticket->title) }}', '{{ addslashes($ticket->description) }}')">
                        <i class="fas fa-check-circle" style="width: 14px;"></i> Selesaikan Konsultasi
                    </a>
                    <a href="#" style="display: flex; align-items: center; gap: 8px; padding: 10px 14px; font-size: 13px; color: #ef4444; text-decoration: none; transition: background 0.2s; border-top: 1px solid #f1f5f9; white-space: nowrap;" onmouseover="this.style.background='#fef2f2'" onmouseout="this.style.background='transparent'" onclick="event.preventDefault(); closeAllDropdowns(); openConfirmCancelModal('{{ $ticket->id }}')">
