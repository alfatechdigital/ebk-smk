...
@section('content')
    ...
    <form class="filter-bar" method="GET">
        <input type="text" name="search" id="search-input" placeholder="Cari nama siswa atau judul"
        ...
        <div class="filter-chips" style="display: flex; gap: 8px; align-items: center; flex-wrap: wrap;">
            <button type="button" class="filter-chip {{ request('favorite') == '1' ? 'active' : '' }}"
                onclick="toggleChip('favorite', this)"
                style="cursor: pointer; padding: 6px 14px; border-radius: 20px; font-size: 13px; font-weight: 600; border: 1px solid #d1d5db; background: #fff; color: var(--slate); transition: all 0.2s; display: inline-flex; align-items: center; gap: 4px; height: 38px; box-sizing: border-box;">
                <i class="fa-solid fa-star" style="font-size: 11px;"></i> Favorit
            </button>
            <button type="button" class="filter-chip {{ request('anonymous') == '1' ? 'active' : '' }}"
                onclick="toggleChip('anonymous', this)"
                style="cursor: pointer; padding: 6px 14px; border-radius: 20px; font-size: 13px; font-weight: 600; border: 1px solid #d1d5db; background: #fff; color: var(--slate); transition: all 0.2s; display: inline-flex; align-items: center; gap: 4px; height: 38px; box-sizing: border-box;">
            ...
            <div class="modal">
                ...
                <form method="POST" action="{{ route('tickets.store') }}">
                    ...
                    <div class="field-group"><label>Topik Konsultasi</label><input type="text" name="title"
                            placeholder="Topik konsultasi" required></div>
                    <div class="field-group"><label>Ceritakan apa yang ingin kamu konsultasikan</label><textarea
                            name="description" placeholder="Tuliskan di sini..." required></textarea></div>
    ...
    <div class="modal-overlay" id="modal-detail-ticket">
        <div class="modal" style="max-width: 600px; max-height: 90vh; overflow-y: auto; padding: 24px;">
            ...
            <div style="display: flex; flex-direction: column; gap: 20px;">
                ...
                <div
                    ...
                    <div>
                        <label
                            ...
                            Layanan</label>
                        <span id="detail-service-badge" class="badge"
                    ...
                    <div>
                        <label
                            style="font-size: 0.7rem; color: #64748b; text-transform: uppercase; font-weight: 700; display: block; margin-bottom: 4px; letter-spacing: 0.5px;">Kode
                            Tiket</label>
                ...
                @if(auth()->user()->isSiswa())
                    <div id="detail-anon-banner-student"
                        style="display: none; align-items: center; gap: 8px; background-color: #1e293b; border: 1px solid #334155; padding: 10px 14px; border-radius: 6px; font-size: 12px; font-weight: 500;">
                ...
                @else
                    <div id="detail-anon-banner-guru"
                        style="display: none; align-items: center; gap: 8px; background-color: #fef2f2; border: 1px solid #fee2e2; color: #dc2626; padding: 10px 14px; border-radius: 6px; font-size: 12px; font-weight: 500;">
                ...
                <div style="text-align: left;">
                    <label
                        style="font-size: 0.75rem; color: #64748b; text-transform: uppercase; font-weight: 700; display: block; margin-bottom: 6px; letter-spacing: 0.5px;">Deskripsi
                        Masalah</label>
                ...
                <div id="detail-cancel-reason-wrapper" style="display: none; text-align: left;">
                    <label
                        style="font-size: 0.75rem; color: #64748b; text-transform: uppercase; font-weight: 700; display: block; margin-bottom: 6px; letter-spacing: 0.5px; color: #ef4444;">Alasan
                        Pembatalan</label>
                    <div style="font-size: 0.925rem; color: #ef4444; background: #fef2f2; border-left: 4px solid #ef4444; padding: 15px; border-radius: 0 8px 8px 0; line-height: 1.6; white-space: pre-wrap;"
                        id="detail-cancel-reason"></div>
                </div>
            </div>
            <div class="modal-footer"
                ...
                @if(auth()->user()->isGuru())
                    <div id="detail-actions-wrapper" class="inline-actions-group">
                        ...
                        <div id="inline-actions-container" class="inline-actions-buttons">
                            <button type="button" id="detail-selesai-btn" class="btn btn-gold"
                                style="display: inline-flex; align-items: center; gap: 4px; margin: 0; padding: 6px 12px; font-size: 12px; white-space: nowrap;">
                ...
                @endif
                <button type="button" class="btn btn-secondary" onclick="closeModal('modal-detail-ticket')"
                    style="margin: 0;">Tutup</button>
                <a href="#" id="detail-chat-btn" class="btn btn-primary"
                    style="margin: 0; display: inline-flex; align-items: center; gap: 4px;"><i class="fas fa-comments"></i>
                    Buka Pesan</a>
            </div>
        </div>
    </div>
