...
<div class="page-header-row">
    ...
    <div style="display: flex; gap: 8px;">
        @if(request('role') === 'siswa' || !request('role'))
            <button class="btn btn-secondary" onclick="openImportModal()"><i class="fas fa-file-excel"></i> Import Siswa</button>
        @endif
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><span class="badge {{ $u->role==='guru'?'badge-success':($u->role==='siswa'?'badge-info':'badge-danger') }}">{{ $u->role_label }}</span></td>
                    <td>
                        @if($u->role === 'siswa')
                            ...
                            @if($u->teacher && $u->teacher->classes->count() > 0)
                                <div style="display: flex; flex-direction: column; gap: 4px;">
                                    @foreach($u->teacher->classes as $cls)
                                        <span class="badge badge-outline" style="border: 1px solid var(--teal); color: var(--teal); text-align: left;">
                    ...
                    <td class="action-btns">
                        <button class="btn btn-secondary btn-sm" onclick="editUser({{ $u->id }},'{{ $u->name }}','{{ $u->email }}','{{ $u->role }}','{{ $u->student?->nis ?? $u->teacher?->nip ?? '' }}','{{ $u->student?->class_id ?? '' }}', {{ $u->teacher ? $u->teacher->classes->pluck('id') : '[]' }})">
                ...
                @empty
                <tr><td colspan="7" class="text-muted" style="text-align:center">Data tidak ditemukan</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<div class="modal-overlay" id="modal-user">
    <div class="modal" style="max-width: 800px; width: 95%;">
        ...
        <form method="POST" id="user-form" action="{{ route('users.store') }}">
            ...
            <div class="form-row">
                <div class="field-group"><label>Nama Lengkap</label><input type="text" name="name" id="user-name" required></div>
            ...
            <div id="container-class-guru" style="display: none; margin-top: 15px;">
                <label style="display: block; margin-bottom: 10px; font-weight: 600;">Pilih Kelas Kelolaan (Guru BK)</label>
                <div class="class-selection-grid">
                    @foreach($classes as $c)
                    <label class="class-card">
                        <div class="class-card-header">
                            <span class="class-name">{{ $c->name }}</span>
                            <input type="checkbox" name="teacher_class_ids[]" value="{{ $c->id }}" class="class-checkbox" id="check-{{ $c->id }}">
                        </div>
                        @if($c->teacher)
                ...
                <p style="font-size: 0.8rem; color: #64748b; margin-top: 12px; display: flex; align-items: center; gap: 6px;">
                    <i class="fas fa-info-circle text-blue-500"></i> Jika kelas sudah memiliki guru, memilihnya akan memindahkan hak akses kelas ke guru ini (ambil alih).
                </p>
            </div>

            <div class="form-row">
                <div class="field-group"><label>Email</label><input type="email" name="email" id="user-email" required></div>
                <div class="field-group"><label>Password</label><input type="password" name="password" id="user-password"></div>
            </div>
        ...
        <form method="POST" action="{{ route('users.import') }}" enctype="multipart/form-data">
            ...
            <div class="form-row">
                <div class="field-group">
                    ...
                    <input type="file" name="file" accept=".xlsx,.xls,.csv" required style="padding: 10px; border: 1px dashed #cbd5e1; border-radius: 8px; width: 100%; background: #f8f9fa;">
                </div>
            </div>
            <div style="margin-top: 15px; font-size: 0.9rem; color: #64748b; background: #f0fdf4; padding: 12px; border-radius: 8px; border: 1px solid #bbf7d0;">
                <p style="margin-bottom: 8px; font-weight: 600; color: #166534;"><i class="fas fa-info-circle"></i> Panduan Format Excel (Baris Pertama)</p>
