    ...
    <form class="filter-bar" method="GET" action="{{ route('catatan.index') }}">
        ...
        <div style="margin-left: auto; display: flex; align-items: center; gap: 12px;">
            ...
            <div class="rekap-dropdown-wrapper" style="position: relative; display: inline-block;">
                <button type="button" class="btn btn-primary" style="margin: 0; display: inline-flex; align-items: center; gap: 6px;" onclick="toggleRekapDropdown(event)">
                    <i class="fas fa-download"></i> Ekspor Rekap <i class="fas fa-chevron-down" style="font-size: 10px;"></i>
                </button>
                <div class="rekap-dropdown-menu" id="rekap-dropdown-menu" style="display: none; position: absolute; right: 0; top: calc(100% + 5px); background: white; border: 1px solid #e2e8f0; border-radius: 8px; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1); z-index: 100; min-width: 140px; overflow: hidden; text-align: left;">
                    <button type="submit" formaction="{{ route('catatan.rekap') }}" style="display: flex; width: 100%; border: none; background: transparent; align-items: center; gap: 8px; padding: 10px 14px; font-size: 12px; color: #334155; text-decoration: none; cursor: pointer; transition: background 0.2s;" onmouseover="this.style.background='#f1f5f9'" onmouseout="this.style.background='transparent'">
                        <i class="fas fa-file-pdf" style="color: #ef4444; width: 14px;"></i> Cetak PDF
                    </button>
                    <button type="submit" formaction="{{ route('catatan.rekap') }}" name="format" value="word" style="display: flex; width: 100%; border: none; background: transparent; align-items: center; gap: 8px; padding: 10px 14px; font-size: 12px; color: #334155; text-decoration: none; cursor: pointer; transition: background 0.2s; border-top: 1px solid #f1f5f9;" onmouseover="this.style.background='#f1f5f9'" onmouseout="this.style.background='transparent'">
        ...
        @forelse ($notes as $note)
            <div class="catatan-item">
                <div class="catatan-date">
                    ...
                    <div style="font-size: 10px; font-weight: 700; opacity: 0.8; margin-top: 2px; line-height: 1;">{{ $note->created_at->format('Y') }}</div>
                </div>
                <div class="catatan-content" style="min-width: 0; display: flex; flex-direction: column; gap: 4px; justify-content: center; flex-grow: 1;">
                    <div class="siswa" style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
                        ...
                        <span style="font-size: 13px; color: var(--slate); font-weight: 600;">{{ $note->ticket?->student?->user?->name ?? 'Anonim' }} · {{ $note->ticket?->student?->class?->name ?? '' }}</span>
                        @if($note->ticket?->service)
                            <span class="badge" style="background: {{ $note->ticket->service->color ?? 'var(--teal)' }}; color: #fff; font-size: 10px; padding: 2px 8px; border-radius: 4px; display: inline-flex; align-items: center; gap: 4px; font-weight: 600; line-height: 1.2;">
                                <i class="{{ $note->ticket->service->icon ?? 'fas fa-tag' }}"></i> {{ $note->ticket->service->name }}
                            </span>
                        @endif
                    </div>
                    <h4 style="margin: 0; font-size: 14px; font-weight: 700; color: var(--charcoal); overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">{{ $note->title }}</h4>
                    <div style="margin: 0; font-size: 13px; color: var(--slate); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="{{ $note->masalah }}">{{ $note->masalah }}</div>
                </div>
                <div class="action-btns" style="display: flex; flex-direction: row; align-items: center; gap: 8px; flex-shrink: 0; margin-left: auto;">
                    <button class="btn btn-secondary btn-sm" style="background: var(--teal); color: white; border-color: var(--teal); margin: 0; display: inline-flex; align-items: center; gap: 4px; padding: 6px 12px; font-size: 12px; font-weight: 600;" title="Detail Catatan" onclick="openNoteDetailModal(this)"
                        ...
                        data-tanggal="{{ $note->created_at->translatedFormat('d M Y') }}"
                    ...
                    <div class="ticket-actions-dropdown" style="position: relative; display: inline-block;">
                        <button type="button" class="btn btn-secondary btn-sm dropdown-trigger" style="padding: 6px 10px; font-size: 12px; margin: 0; display: inline-flex; align-items: center; justify-content: center; height: 28px; background: transparent; border: 1px solid var(--slate-light, #cbd5e1); color: var(--slate);" onclick="toggleNoteActionsDropdown(event, '{{ $note->id }}')">
                            <i class="fas fa-ellipsis-v"></i>
                        </button>
                        <div class="dropdown-menu-content" id="note-actions-dropdown-{{ $note->id }}" style="display: none; position: absolute; right: 0; top: calc(100% + 5px); background: white; border: 1px solid #e2e8f0; border-radius: 8px; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05); z-index: 100; min-width: 170px; overflow: hidden; text-align: left;">
                            <a href="#" style="display: flex; align-items: center; gap: 8px; padding: 10px 14px; font-size: 13px; color: #334155; text-decoration: none; transition: background 0.2s;" onmouseover="this.style.background='#f1f5f9'" onmouseout="this.style.background='transparent'" onclick="event.preventDefault(); closeAllDropdowns(); openEditModalFromData('{{ $note->id }}', '{{ addslashes($note->title) }}', '{{ addslashes($note->masalah) }}', '{{ addslashes($note->tindakan) }}', '{{ addslashes($note->kesimpulan ?? '') }}', '{{ $note->ticket?->service_id ?? '' }}')">
                                <i class="fas fa-edit" style="width: 14px; color: var(--slate);"></i> Edit Catatan
                            </a>
                            <a href="{{ route('catatan.pdf', $note) }}" style="display: flex; align-items: center; gap: 8px; padding: 10px 14px; font-size: 13px; color: #334155; text-decoration: none; transition: background 0.2s; border-top: 1px solid #f1f5f9;" onmouseover="this.style.background='#f1f5f9'" onmouseout="this.style.background='transparent'">
                                <i class="fas fa-file-pdf" style="width: 14px; color: #ef4444;"></i> Cetak PDF
                            </a>
                            <a href="{{ route('catatan.pdf', [$note, 'format' => 'word']) }}" style="display: flex; align-items: center; gap: 8px; padding: 10px 14px; font-size: 13px; color: #334155; text-decoration: none; transition: background 0.2s; border-top: 1px solid #f1f5f9;" onmouseover="this.style.background='#f1f5f9'" onmouseout="this.style.background='transparent'">
                                <i class="fas fa-file-word" style="width: 14px; color: #2b579a;"></i> Ekspor Word
                            </a>
                            <a href="#" style="display: flex; align-items: center; gap: 8px; padding: 10px 14px; font-size: 13px; color: #ef4444; text-decoration: none; transition: background 0.2s; border-top: 1px solid #f1f5f9;" onmouseover="this.style.background='#fef2f2'" onmouseout="this.style.background='transparent'" onclick="event.preventDefault(); closeAllDropdowns(); openDeleteModal('{{ $note->id }}')">
...
<div class="modal-overlay" id="modal-confirm-edit-catatan">
    <div class="modal" style="max-width: 400px; text-align: center; padding: 24px;">
        ...
        <div class="modal-footer" style="justify-content: center; gap: 10px; border-top: none; padding-top: 20px; margin-top: 10px;">
            <button type="button" class="btn btn-secondary" onclick="closeModal('modal-confirm-edit-catatan')" style="margin: 0;">Batal</button>
            <button type="button" class="btn btn-primary" onclick="submitEditForm()" style="margin: 0; background: var(--teal); border-color: var(--teal);"><i class="fas fa-check"></i> Ya, Simpan</button>
        </div>
    </div>
</div>
        ...
        <div style="display: flex; flex-direction: column; gap: 20px;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 15px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 15px; text-align: left;">
                ...
                <div>
                    ...
                    <div>
                        <span id="detail-siswa-layanan" class="badge" style="color: #fff; padding: 4px 10px; border-radius: 4px; font-weight: 700; font-size: 11px; display: inline-flex; align-items: center; gap: 6px;">-</span>
                    </div>
                </div>
            </div>
            ...
            <div style="text-align: left;">
                <label style="font-size: 0.75rem; color: #64748b; text-transform: uppercase; font-weight: 700; display: block; margin-bottom: 6px; letter-spacing: 0.5px;">Masalah / Permasalahan</label>
                <div style="font-size: 0.925rem; color: #334155; background: #f8fafc; border-left: 4px solid var(--teal); padding: 15px; border-radius: 0 8px 8px 0; line-height: 1.6; white-space: pre-wrap;" id="detail-masalah"></div>
            </div>

            <div style="text-align: left;">
                <label style="font-size: 0.75rem; color: #64748b; text-transform: uppercase; font-weight: 700; display: block; margin-bottom: 6px; letter-spacing: 0.5px;">Tindakan yang Dilakukan (Solusi)</label>
                <div style="font-size: 0.925rem; color: #334155; background: #f8fafc; border-left: 4px solid #059669; padding: 15px; border-radius: 0 8px 8px 0; line-height: 1.6; white-space: pre-wrap;" id="detail-tindakan"></div>
            </div>

            <div style="text-align: left;">
                <label style="font-size: 0.75rem; color: #64748b; text-transform: uppercase; font-weight: 700; display: block; margin-bottom: 6px; letter-spacing: 0.5px;">Kesimpulan</label>
                <div style="font-size: 0.925rem; color: #334155; background: #f8fafc; border-left: 4px solid #f59e0b; padding: 15px; border-radius: 0 8px 8px 0; line-height: 1.6; white-space: pre-wrap;" id="detail-kesimpulan"></div>
            </div>
        </div>
